...
public:
    ...
    int dig(int r, int rev, vector<int> &memo) {
        int prv2 = INF, prv1 = 0, curr;
        int goal = r;
        Edge *prev = nullptr;

        while (true) {
            memo[r] = 1;
            curr = min(g[r][0].y, g[r][1].y) + 1 + prv1;
            if (prev)
                curr = min(curr, max(prev->x, prev->y) + 1 + prv2);
    ...
    int solve(vector <int> Type1, vector <int> Number1, vector <int> Type2, vector <int> Number2) {
        ...
        for (int i=0; i<N; ++i)
            if (memo1[i] == -1)
                ans += min(dig(i, 0, memo1), dig(i, 1, memo2));
