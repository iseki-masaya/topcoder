...
public:
	...
	bool is_in(string &s) {
		return "1200" <= s && s <= "3923";
	}
	
	vector<int> parse(string &s, int &Elly) {
		string delimiter = " ";
		vector<int> res;
		int p;
		while ( (p = (int)s.find(delimiter)) != string::npos) {
			string rate = s.substr(0,p);
			if (is_in(rate))
				res.push_back(conv(rate));
			s = s.substr(p+1);
		}
		if (!s.empty() && is_in(s))
			res.push_back(conv(s));
		Elly = res[0];
		return res;
	}
	
	double getAverage(vector <string> ratings) {
		string r = accumulate(ratings.begin(), ratings.end(), string(""));
		int Elly = 0;
		vector<int> rs = parse(r, Elly);
		...
		double esum = 0.0, ans = -100.0;
		bool includeElly = false;
		for (int i=0; i<N; i+=R) {
			double expect = 0.0;
			int size = min(R,N-i);
			for (int j=0; j+i<N && j<R; ++j) {
				int k = i+j;
				if (rs[k] == Elly) {
					includeElly = true;
					expect = rs[k];
				...
				expect += (double)rs[k]/size;
			}
			esum += expect;
			if (i/R == M-2 && !fit && includeElly)
				ans = esum/(M-1);
		}
		if (ans > 0.0)
			ans = esum/M * ( (double)(N%R)/R ) + ans * ( (double)(R-(N%R))/R );
		else
			ans = esum/M;
