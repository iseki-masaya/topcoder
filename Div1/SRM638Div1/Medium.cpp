...
public:
    ...
    int count(vector <int> size, int maxSizeSum) {
        ...
        if (N <= 1)
            return 1;
        auto mn = min_element(size.begin(), size.end());
        auto mx = max_element(size.begin(), size.end());
        int res;
        if (*mx + *mn <= maxSizeSum) {
            size.erase(mn);
            res = ( (long long)N * count(size, maxSizeSum) )%MOD;
        } else {
            int mxi = (int)distance(size.begin(), mx);
            vector<int> a(size.begin(), size.begin()+mxi);
            vector<int> b(size.begin()+mxi+1, size.end());
            res = ( (long long)count(a, maxSizeSum) * count(b, maxSizeSum) )%MOD;
        }
        return res;
    }
