...
public:
    void
    push(vector<string> &ans,long long m)
    {
        ostringstream os;
        os << m << ".mp3";
    ...
    {
        vector<string> ans;
        for (int i=1; i<=n && i<10 && ans.size()!=50; ++i) {
            long long k = i;
            string a;
            while (k<=n && ans.size()!=50) {
                push(ans, k);
                k *= 10;
            }
            k /= 10;
            while (i < k && ans.size()!=50) {
                for (int j=1; j+k<=n && j<10 && ans.size()!=50; ++j) {
                    push(ans, k+j);
                    long long t = k+j;
                    t *= 10;
                    for (int l=0; l+t<=n && l<10 && ans.size()!=50; ++l) {
                        push(ans, l+t);
                    }
                }
                k /= 10;
