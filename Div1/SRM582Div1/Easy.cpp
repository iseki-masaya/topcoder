...
public:
    ...
    long long minimalFatigue(vector <int> magicalGirlStrength, vector <int> enemyStrength, vector<long long> enemyCount) {
        int N = (int)magicalGirlStrength.size();
        vector<long long> fat(N,0);
        sort(magicalGirlStrength.begin(), magicalGirlStrength.end());
        int M = (int)enemyCount.size();
        for (int i=0; i<M; ++i) {
            int p = (int)distance(magicalGirlStrength.begin(), lower_bound(magicalGirlStrength.begin(), magicalGirlStrength.end(), enemyStrength[i]));
            if (p == N) {
                return -1;
            }
            fat[p] += enemyCount[i];
        ...
        while (true) {
            ...
            for (int i=N-1; i>0; --i) {
                if (fat[i-1] > fat[i]) {
                    long long sub = (fat[i-1] - fat[i] + 1) / 2;
                    fat[i-1] -= sub;
                    fat[i] += sub;
                    update = true;
                }
        ...
        return *max_element(fat.begin(), fat.end());
    }
