...
public:
    ...
    double getAverage(vector <int> ratings)
    {
    ...
    vector<double> groupAverage;
    int i = 0;
    while (i < N) {
            // Most groups have R coders, the last one might have N%R coders
            int j = std::min(i + R, N);
            if (i <= elly && elly < j) { //Elly is in this group,
                //Elly's room will always pick Elly:
                groupAverage.push_back(ellysRating);
            } else {
                ...
                groupAverage.push_back( sum / (j - i) );
            }
            i = j;
    }
    ...
    double
    getAverage(vector <string> ratings)
    {
        vector<int> r;
        string str = "";
        for (int i=0; i<ratings.size(); ++i) {
            str.append(ratings[i]);
        }
        split(str, r);
        ...
        int elly_pos = 0;
        for (int i=0,j=0; i<N; i += R) {
            double sum = 0;
            for (j=0; j<R && i+j<N; ++j) {
                sum += r[i+j];
            }
            if (r[i] >= elly && elly >= r[i+j-1]) {
                elly_pos = i;
                avg_vec.push_back(elly);
            }
            else {
                avg_vec.push_back((double)sum/j);
            }
        }
        ...
        else if (elly_pos+R > N) {
            res = sum_all/(T+1);
        }
        else {
            double p1 = (N%R)/(double)R;
            double p0 = 1.0 - p1;
            res = p1*(sum_all/(T+1)) + p0*(sum_all-(double)avg_vec[T])/T ;
        }
